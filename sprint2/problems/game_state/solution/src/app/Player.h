...
class Player {
 public:
  ...
  Player(std::shared_ptr<model::GameSession> gs, std::shared_ptr<model::Dog> dog, Token token) : session_(std::move(gs)), dog_(std::move(dog)), token_(std::move(token)), id_(session_->GetId(), dog_->GetId()) {
  }

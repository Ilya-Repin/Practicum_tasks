...
Game LoadGame(const std::filesystem::path &json_path) {
  ...
  if (value.as_object().contains("defaultDogSpeed")) {
    default_dog_speed = GetDoubleValue(value.as_object(), "defaultDogSpeed");
  }
  ...
  for (auto& element : maps_array) {
    ...
    if (map_object.contains("dogSpeed")) {
      dog_speed = GetDoubleValue(map_object, "dogSpeed");
    }

    Map map(Map::Id{GetStringValue(map_object, JsonAttributes::ID)}, GetStringValue(map_object, JsonAttributes::NAME), dog_speed);

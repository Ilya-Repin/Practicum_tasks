...
std::shared_ptr<boost::json::array> ApiHandler::MakeRoadsArr(const model::Map *map) {
...
std::shared_ptr<boost::json::array> ApiHandler::MakeBuildingsArr(const model::Map *map) {
...
std::shared_ptr<boost::json::array> ApiHandler::MakeOfficesArr(const model::Map *map) {
...
void ApiHandler::ConfigureJsonMap(boost::json::object &map_obj, const model::Map *map) {
  map_obj[MakeBoostSV(JsonAttributes::ID)] = *map->GetId();
  map_obj[MakeBoostSV(JsonAttributes::NAME)] = map->GetName();
...
std::variant<JsonResponse,  std::shared_ptr<app::Token>> ApiHandler::GetToken(const http::request<http::string_body> &req) {

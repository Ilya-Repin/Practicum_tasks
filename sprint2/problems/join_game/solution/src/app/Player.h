...
class Player {
 public:
  using Id = std::pair<model::Map::Id, model::Dog::Id>;

  Player(std::shared_ptr<model::GameSession> gs, std::shared_ptr<model::Dog> dog, Token token) : session_(std::move(gs)), dog_(std::move(dog)), token_(std::move(token)), id_(session_->GetMapId(), dog_->GetId()) {
  }
...
class Players {
 public:
  ...
  // Getters
  std::shared_ptr<Player> GetByIdAndMapId(model::Dog::Id dog_id, model::Map::Id map_id);
 ...
 private:
  ...
  struct PlayerIdHash {
    std::size_t operator()(const std::pair<model::Map::Id, model::Dog::Id>& p) const {
      std::size_t string_hash = std::hash<std::string>{}(*p.first);
      std::size_t uint64_hash = std::hash<uint64_t>{}(*p.second);
      return string_hash ^ (uint64_hash << 1);
    }
  };

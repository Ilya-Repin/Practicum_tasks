    ...
    std::string ConvertTagsToString(const std::set<std::string>& tags)  {
      std::string str;
    ...
    bool View::EditAuthor(std::istream& cmd_input) const {
      try {
        ...
        if (e.what() == "Invalid author num: #"s) {
          return true;
        }
        if (e.what() == "Author deleted."s) {
          std::cout << "AUTHOR DELETED" << std::endl;
        }
      }
    ...
    std::optional<std::string> View::GetAuthorId(std::istream& cmd_input) const {
      ...
      if (author_name.empty()) {
        ...
        author_id = use_cases_.GetAuthorIdFromName(author_name);
      }
    ...
    bool View::ShowBook(std::istream& cmd_input) const {
      try {
        ...
        if (!book.tags.empty()) {
          output_ << "Tags: "sv << ConvertTagsToString(book.tags) << std::endl;
        }
      } catch (const std::exception& e) {
    ...
    bool View::EditBook(std::istream& cmd_input) const {
      ...
      try {
        ...
        output_ <<  "Enter tags (current tags: " << ConvertTagsToString(book.tags)
                << "):" << std::endl ;
    ...
    std::optional<std::string> View::SelectAuthor() const {
      ...
      try {
        ...
        throw std::runtime_error("Invalid author num: " + str);
      }
      --author_idx;
    ...
    std::optional<std::string> View::EnterAuthor() const {
      ...
      try{
        return use_cases_.GetAuthorIdFromName(name);
      } catch (const std::exception&) {
      }
    ...
    std::vector<detail::AuthorInfo> View::GetAuthors() const {
      std::vector<detail::AuthorInfo> dst_authors = use_cases_.GetAuthors();
      return dst_authors;
    }

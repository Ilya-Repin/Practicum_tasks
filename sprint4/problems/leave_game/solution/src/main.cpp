...
int main(int argc, const char *argv[]) {
  ...
  try {
    if (auto args = util::ParseCommandLine(argc, argv)) {
      ...
      std::pair<std::size_t, std::string> db_config{num_threads, GetConfigFromEnv()};
      model::Game game = json_loader::LoadGame(args.value().config_file);
      auto serializing_listener = std::make_shared<infrastructure::SerializingListener>(args.value().state_file,
                                                                                        std::chrono::milliseconds(args.value().save_state_period));
      auto app =
          std::make_shared<app::Application>(game, args.value().randomize_spawn_point, serializing_listener, db_config);

      serializing_listener->LoadState(app);
      ...
      if (!args.value().state_file.empty() && args.value().save_state_period != 0) {
        serializing_listener->SaveState(app);
      }
    }

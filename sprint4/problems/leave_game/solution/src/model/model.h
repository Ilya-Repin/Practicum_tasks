...
class Map {
 public:
  ...
  Map(Id id, std::string name, double speed, int bag_capacity, ExtraData extra_data) noexcept
      : id_(std::move(id)),
        name_(std::move(name)),
        dog_speed_(speed),
  ...
  const std::vector<uint64_t> &GetValues() const noexcept {
    return values_for_loot_;
  }
  ...
  void AddLootValue(uint64_t value);
 ...
 private:
  ...
  double dog_speed_;
  int bag_capacity_;
  std::vector<uint64_t> values_for_loot_;
  std::string name_;
...
class Game {
 public:
  ...
  Game(LootGeneratorConfig config, double dog_retirement_time)
      : config_(config), dog_retirement_time_(dog_retirement_time) {
  }

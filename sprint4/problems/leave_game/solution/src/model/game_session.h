...
class GameSession {
 public:
  ...
  // Constructor
  GameSession(const model::Map &map, Id id, bool is_spawn_random, const LootGeneratorConfig &config,
              uint64_t dog_retirement_time, Dogs &&dogs = {}, Loots &&loots = {})
      : map_(map),
        id_(id),
        is_spawn_point_random_(is_spawn_random),
    ...
    loots_amount_ = loots_.size();
    players_amount_ = dogs_.size();
  }
  ...
  std::vector<std::shared_ptr<Dog>> Tick(uint64_t time_delta);
 ...
 private:
  ...
  size_t MAX_PLAYERS_AMOUNT = 15;
  uint64_t dog_retirement_time_;
};

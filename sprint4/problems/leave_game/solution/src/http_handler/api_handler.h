...
class ApiHandler {
 public:
  ApiHandler(std::shared_ptr<app::Application> app, util::Args &args) : app_(app), manual_tick(!args.tick_period) {
  }
 ...
 private:
  ...
  StringResponse GetRecords(const http::request<http::string_body> &req);
};

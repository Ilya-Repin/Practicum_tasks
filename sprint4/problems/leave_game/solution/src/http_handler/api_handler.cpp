...
std::shared_ptr<boost::json::array> ApiHandler::MakeRecordsArr(std::string target) {
  int offset = 0;
  int limit = 100;

  if (target.find("start=") != std::string::npos && target.find("&maxItems=") != std::string::npos) {
    size_t start_pos = target.find("start=");
    size_t end_pos = target.find("&maxItems=");
    std::string start_value =
        target.substr(start_pos + 6, end_pos - start_pos - 6);

    size_t max_items_pos = target.find("maxItems=");
    std::string max_items_value = target.substr(max_items_pos + 9);

    offset = std::stoi(start_value);
    limit = std::stoi(max_items_value);
  }

  if (limit > 100) {
    throw std::invalid_argument("maxItems bigger than 100!");
  }
  ...
  for (auto record : records) {
    ...
    record_obj[MakeBoostSV(JsonAttributes::NAME)] = record.name;
    record_obj[MakeBoostSV(JsonAttributes::SCORE)] = record.score;
    record_obj[MakeBoostSV(JsonAttributes::PLAY_TIME)] =
        std::floor(static_cast<double>(record.play_time_ms) / 100) / 10;
...
StringResponse ApiHandler::GetRecords(const http::request<http::string_body> &req) {
  if (req.method() != http::verb::get && req.method() != http::verb::head) {
    return MakeWrongMethodError(HttpResponseConstants::GET_HEAD_EXPECTED,
                                HttpResponseConstants::ALLOWED_GET_HEAD,
                                req.version());
  } else {
    std::shared_ptr<boost::json::array> response_arr;

    try {
      response_arr = MakeRecordsArr(std::string(req.target()));
    } catch (std::exception &e) {
      return MakeBadRequestError("Too much allItems"sv, req.version());
    }
...
StringResponse ApiHandler::HandleApiRequest(const http::request<http::string_body> &req) {
  ...
  } else if (target.find(SystemPaths::RECORDS) != std::string::npos) {

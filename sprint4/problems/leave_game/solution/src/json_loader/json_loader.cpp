...
model::LootGeneratorConfig GetLootGeneratorConfig(boost::json::value &value) {
  ...
  return {std::chrono::milliseconds(static_cast<int>(1000
      * GetDoubleValue(loot_config, JsonAttributes::LOOT_CONFIG_PERIOD))),
...
Game LoadGame(const std::filesystem::path &json_path) {
  ...
  if (value.as_object().contains(MakeBoostSV(JsonAttributes::DEFAULT_RETIREMENT_TIME))) {
    dog_retirement_time = GetDoubleValue(value.as_object(), JsonAttributes::DEFAULT_RETIREMENT_TIME);
  }

  Game game(GetLootGeneratorConfig(value), dog_retirement_time * 1000);

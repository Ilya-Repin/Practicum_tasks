...
int main(int argc, const char *argv[]) {
  ...
  try {
    if (auto args = util::ParseCommandLine(argc, argv)) {
      ...
      auto serializing_listener = std::make_shared<infrastructure::SerializingListener>(args.value().state_file, std::chrono::milliseconds(args.value().save_state_period));

      auto app = std::make_shared<app::Application>(game, args.value().randomize_spawn_point, serializing_listener);

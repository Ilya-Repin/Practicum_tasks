...
std::pair<std::vector<GameState>, std::vector<model::Loot>> Application::GetGameState(Token token) const {
  if (!players_.GetByToken(token)) {
    return {};
  }
  ...
  for (auto &dog : dogs) {
    ...
    game_state.push_back({dog.first, dog_ptr->GetPosition(), dog_ptr->GetVelocity(),

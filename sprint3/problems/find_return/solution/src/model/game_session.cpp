...
void GameSession::AddLoot(uint64_t time_delta) {
  ...
  for (int i = 0; i < new_loot_amount; ++i) {
    ...
    loots_.insert({loots_amount_++, {random_type, spawn_position.x,
                                     spawn_position.y}});
  }
}
...
//    1) Создаем item_gatherer
  ItemGatherer item_gatherer;
//    2) Добавляем туда собачек
...
//    5) Применяем функцию FindGathererEvents

  auto gathering_events = FindGatherEvents(item_gatherer);

//    6) Обрабатываем события, полученные в 5 пункте. Там два кейса: лут или база

...
std::pair<std::vector<GameState>, std::unordered_map<uint64_t, model::Loot>> Application::GetGameState(Token token) const {
  if (!players_.GetByToken(token)) {
    return {};
  }
  ...
  for (auto &dog : dogs) {
    ...
    game_state.push_back({dog.first, dog_ptr->GetPosition(), dog_ptr->GetVelocity(), dog_ptr->GetBag(),
...
std::shared_ptr<model::GameSession> Application::AddGameSession(const model::Map &map) {
  ...
  std::shared_ptr<GameSession>
      session_ptr = std::make_shared<GameSession>(map, GameSession::Id(sessions_counter), is_spawn_point_random_, game_.GetLootGeneratorConfig());

...
std::string SayHello(const StringRequest &request) {
  std::string greeting = "Hello, "s + std::string(request.target()).substr(1);
  return greeting;
}
...
StringResponse HandleRequest(StringRequest&& req) {
  ...
  if (req.method() == http::verb::get) return text_response(http::status::ok, std::string_view(SayHello(req)));
